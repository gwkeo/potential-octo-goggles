...
def impost(s):
    ...
    if e != 0: # xy существует, это параллельные прямые
        ...
        if B == 0: # пересекающиеся прямые
            ...
            main_solution.center = Point(f"{var1}", f"{var2}")
            ...
            main_solution.center = Point(f"{x0}", f"{y0}")
            
            if int(B / abs(B)) == 1: # если итог = 1 
                e0 = c0/a0
                main_solution.focus1 = Point(f"{c0+x0}", f"{y0}")
                main_solution.focus2 = Point(f"{-c0+x0}", f"{y0}")
                main_solution.eccenter = f"{e0}" 
                ...
                e0 = c0/b0
                main_solution.focus1 = Point(f"{x0}", f"{c0+y0}")
                main_solution.focus2 = Point(f"{x0}", f"{-c0+y0}") 
                main_solution.eccenter = f"{e0}"
        ...
        if a == 0: # вид параболы (y-y0)^2=2p(x-x0)
            ...
            main_solution.focus1 = Point(f"{int1(p/2+x0)}", f"{y0}")
        ...
        if c == 0: # вид параболы (x-x0)^2=2p(y-y0)
            ...
            main_solution.focus1 = Point(f"{x0}", f"{int1(p/2+y0)}")                               
            main_solution.direct1 = f"y = {int1(y0 - p/2)}"    

        main_solution.parameter = f"{abs(p)}"
        main_solution.center = Point(f"{x0}", f"{y0}") # конечная информация для всех парабол
